...
const Home = () => {
    ...
    const handleCreate = async () => {
        try{
            await axios.post("/api/v1/create/add-text", {title: addTitle, discription: addDiscription});//endpoint for creating
            console.log("added")
        }catch(err){
            console.log(err);
        }
    ...
    const handleEdit = async (value:any) =>{
        try{
            const iddata = value;
            await axios.post("/api/v1/update/update-this",{iddata: iddata, payload: {title: editTitle, discription: editDiscription}});//endpoint for editing
        }catch(err){
            console.log(err);
        }
    }
    const handleRead = async () =>{
        try{
            const read = await axios.get("/api/v1/read/get-data");//endpoint for read
    ...
    const handleDelete = async (value:any)=>{
        const iddata = value;
        try{
            await axios.post("/api/v1/delete/delete-this",{iddata: iddata});//endpoint for delete
            handleRead();
        }catch(err){
            console.log(err);
        }
